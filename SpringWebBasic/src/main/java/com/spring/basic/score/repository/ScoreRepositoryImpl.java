...
public class ScoreRepositoryImpl implements IScoreRepository {
	...
	static {
		SCORE_MAP = new HashMap</* 타입 생략 가능 Integer, Score */>();
...
//		SCORE_MAP.keySet(); // 키 값
//		SCORE_MAP.values(); // 벨류 값
		// SCORE_MAP은 번호가 key, 객체가 value로 이루어져 있음.
		// 객체들만 전부 뽑아서 List로 만들 것 이기 때문에
		// SCORE_MAP에서 value들만 전부 뽑아낸 뒤, ArrayList의 생성자의 매개값으로 전달해서 List로 포장
		List<Score> scoreList = new ArrayList<>(SCORE_MAP.values());
		return scoreList;
	}
	...
	@Override
	public void updateScore(Score modScore) {
